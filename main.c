...
long int	g_init_time = 0;							// Absolute (in ms from beggining) actual state's staRting time
...
int ExperimentFSA(void){
	...
	//if (g_trial_state > TRIAL_STATE_INIT_CONTACT)
	//	ExperimentSaveData(x1,y1,press,getmicroseconds());
	...
	switch(g_trial_state)
	{
		...
		case TRIAL_STATE_PRECHANGE://////////////////////////////////////////////
			...
			else if (g_init_time >= g_config.time_of_jump*1000)
			{
		...
		case TRIAL_STATE_CONTACT1://///////////////////////////////////////
			...
			else if (time_inc > g_config.time_limit*1000)
			{
